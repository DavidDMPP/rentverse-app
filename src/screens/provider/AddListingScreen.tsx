 ...
 * AddListingScreen
 * 
 * Form screen for creating a new property listing.
 * Collects property details, images, and amenities.
 * 
...
// Fallback property types if API fails - these won't work for submission
// but will show the user what types are expected
const FALLBACK_PROPERTY_TYPES: PropertyType[] = PROPERTY_TYPES.map((name, index) => ({
  id: `fallback-${index}`,
 ...
 * Requirements: 6.1, 6.4
 */
export function AddListingScreen(): React.JSX.Element {
  ...
  /**
   * Fetch property types and amenities
   */
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log('Fetching property types and amenities...');
        const [typesData, amenitiesData] = await Promise.all([
          getPropertyTypes().catch(err => {
            console.error('Error fetching property types:', err);
            return [];
          }),
          getAmenities().catch(err => {
            console.error('Error fetching amenities:', err);
            return [];
          }),
        ]);
        console.log('Property types:', typesData);
        console.log('Amenities:', amenitiesData);
        
        // Use fetched data or fallback
        const types = Array.isArray(typesData) && typesData.length > 0 
          ? (typesData as PropertyType[]) 
          : FALLBACK_PROPERTY_TYPES;
        setPropertyTypes(types);
        setUsingFallbackTypes(!(Array.isArray(typesData) && typesData.length > 0));
        setAmenities(Array.isArray(amenitiesData) ? (amenitiesData as Amenity[]) : []);
      } catch (err) {
        console.error('Error fetching form data:', err);
        // Use fallback property types
  ...
  /**
   * Add image URL (placeholder for image picker)
   */
  ...
  const handleSubmit = useCallback(async () => {
    // Check if using fallback property types (database not seeded)
    if (form.propertyTypeId.startsWith('fallback-')) {
      const errorMsg = 'Property types not loaded from server. Please ensure the backend database is seeded with property types.\n\nRun: npm run db:seed (in backend folder)';
      if (Platform.OS === 'web') {
        window.alert(errorMsg);
      } else {
        Alert.alert('Database Not Ready', errorMsg);
      }
      return;
    }

    // Build request data
    // Status is set to 'APPROVED' directly since admin is creating the listing
    ...
    try {
      ...
      if (error.status === 404) {
        errorMessage = 'Create listing feature is not available yet. Backend endpoint not implemented.';
    ...
    >
      ...
      >
        ...
        <View style={[styles.header, { backgroundColor: theme.colors.surface }] }>
          ...
          <Text style={[styles.headerTitle, { color: theme.colors.text }]}>Add New Listing</Text>
          <View style={{ width: 40 }} />
        </View>

        {/* Warning banner if using fallback property types */}
        {usingFallbackTypes && (
          <View style={[
            styles.warningBanner,
            { backgroundColor: isDark ? 'rgba(245, 158, 11, 0.12)' : '#FEF3C7', borderColor: isDark ? 'rgba(245, 158, 11, 0.3)' : '#F59E0B' },
          ]}>
            <Ionicons name="warning" size={20} color={Colors.warning} />
            <Text style={[styles.warningText, { color: isDark ? Colors.accent : '#92400E' }]}>
              Property types not loaded from server. Database may need seeding. Run: npm run db:seed
            </Text>
          </View>
        )}
        ...
        >
          {isSubmitting ? (
            ...
            <>
              ...
              <Text style={[styles.submitButtonText, { color: Colors.white }]}>Create Listing</Text>
            </>
          )}
        </TouchableOpacity>
      </ScrollView>
